use bevy::math::DVec3;
use crate::arm::arm3d::ArmSim3d;

/// How close the EE needs to get before we count a waypoint as "reached".
...
impl WaypointSeq {
    ...
    /// Kick off the very first waypoint move. Call once after creation to start immediately.
    pub fn start(&mut self, arm: &mut ArmSim3d) {
    ...
    /// Drive the sequence forward.  Call every frame with current EE-to-target distance and dt.
    pub fn advance(&mut self, arm: &mut ArmSim3d, dist_to_target: f64, dt: f64) {
        ...
        if self.dwelling {
            // Wait out the dwell, then advance to next waypoint.
            self.dwell_timer -= dt;
            ...
            // Reached the waypoint — start dwelling.
            let (_, dwell) = self.waypoints[self.current_idx];
            if dwell > 0.0 {
                self.dwelling    = true;
                self.dwell_timer = dwell;
            } else {
                // Zero dwell — go straight to next.
                self.current_idx += 1;
                if self.current_idx < self.waypoints.len() {
                    let (next, _) = self.waypoints[self.current_idx];
                    arm.start_cartesian_move(next, self.speed_m_s);
    ...
    /// The current target position (used by the caller to compute dist_to_target).
...
/// Sample a figure-8 (Lissajous 1:2) in the XY plane.
/// x = cx + ax * sin(t),  y = cy + ay * sin(2t)
...
/// 16 waypoints, stays within comfortable arm reach.
...
pub fn circle_sweep(speed: f64) -> WaypointSeq {
    ...
    let y  = 0.8_f64;
    let r  = 0.5_f64;
    let waypoints = (0..n).map(|i| {
        let t = std::f64::consts::TAU * i as f64 / n as f64;
        let p = DVec3::new(cx + r * t.cos(), y, cz + r * t.sin());
        (p, 0.0)
    }).collect();
    WaypointSeq::new(waypoints, speed, "Circle Sweep")
}

/// Helix: circle of radius 0.4 around (0.8, y, 0.0) spiralling from y=0.4 to y=1.2.
/// Two full turns over 16 waypoints.
pub fn helix(speed: f64) -> WaypointSeq {
    ...
    let waypoints = (0..n).map(|i| {
        ...
        let t    = std::f64::consts::TAU * 2.0 * i as f64 / n as f64; // two full turns
        let y    = y_start + (y_end - y_start) * frac;
        let p    = DVec3::new(cx + r * t.cos(), y, r * t.sin());
        (p, 0.0)
    }).collect();
    WaypointSeq::new(waypoints, speed, "Helix")
}

/// Wave hello: arm rises to shoulder height then oscillates in Z (like a friendly hand wave).
pub fn wave_hello(speed: f64) -> WaypointSeq {
    ...
    let mut waypoints = vec![
        (DVec3::new(0.8, 0.9, 0.0), 0.2),  // approach from below
        (centre, 0.15),                      // arrive at wave position
    ];
    // Four oscillations (right → left → right → left → right)
    for i in 0..4 {
        waypoints.push(if i % 2 == 0 { (right, 0.0) } else { (left, 0.0) });
    }
    waypoints.push((right, 0.0));
    waypoints.push((centre, 0.3));  // settle back

    WaypointSeq::new(waypoints, speed, "Wave Hello")
}

/// Simulate picking an object from one spot and placing it at another.
/// pick-floor → lift → swing → lower → place
pub fn pick_and_place(speed: f64) -> WaypointSeq {
    ...
    let waypoints = vec![
        (pick_floor,  0.5),  // arrive at pick location, dwell to "grasp"
        (pick_lift,   0.1),  // lift
        (place_lift,  0.1),  // swing across
        (place_floor, 0.5),  // lower and "release"
    ];
...
/// Trace a triangle in 3D space — three corners with a pause at each vertex.
/// Good for verifying straight-line Cartesian moves between very different poses.
...
/// Knock on the table: arm drops to near-floor, taps three times, then retreats.
...
/// Write a rough "Z" in the vertical XZ plane — three strokes of the letter.
/// (nod to Zorro: three slashes with a pause to savour each one)
pub fn zorro(speed: f64) -> WaypointSeq {
    let waypoints = vec![
        (DVec3::new(0.65, 1.1, -0.35), 0.1),  // top-left corner
        (DVec3::new(0.65, 1.1,  0.35), 0.1),  // top-right (horizontal stroke)
        (DVec3::new(0.65, 0.5, -0.35), 0.1),  // bottom-left (diagonal slash)
        (DVec3::new(0.65, 0.5,  0.35), 0.3),  // bottom-right (horizontal stroke)
    ];
    WaypointSeq::new(waypoints, speed, "Zorro Z")
}

/// A slow, dreamy figure-eight at reduced speed with micro-dwells — feels contemplative.
