...
use crate::{
    ...
    motor::motor_sim::{MotorParams, MotorSim},
...
const IK_TOLERANCE_M: f64   = 0.002;
...
fn main() {
    ...
    App::new()
        ...
        .insert_resource(UiState { selected_joint: 0 })
...
fn update_status(
    ...
    text.0 = format!(
        "[{sim_s}]  FPS:{fps:5.0}  t:{:.2}s{ik_s}\n\
         ...
         -----------------------------------------\n\
         WASD/QE: move target   Space: pause   R: reset\n\
         LDrag: orbit camera    Scroll: zoom",
        state.elapsed_s,
        ee.x, ee.y, ee.z,
    ...
    egui::SidePanel::right("charts_panel")
        ...
        .show(ctx, |ui| {
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                ui.separator();
                ui.label(egui::RichText::new("WASD/QE: move target  Space: pause  R: reset").weak().small());
            });
        });
}
